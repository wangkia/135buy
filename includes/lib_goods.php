 ...
 * @return  array
 */
function get_recommend_goods($type = '', $cats = '', $limit = 0)
{
    ...
    switch ($type)
    {
        case "hot":$type_sql = " and g.is_hot = 1 ";
            break;
        case "best":$type_sql = " and g.is_best = 1 ";
            break;
        case "new":$type_sql = " and g.is_new = 1 ";
            break;
    }
    if ($cats !== "")
    {
        $cats_sql = " and cat_id =" . intval($cats);
    }
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_img, g.goods_thumb, g.sort_order,g.shop_price,g.market_price,'.
            'g.promote_price,g.promote_start_date,g.promote_end_date,g.goods_brief' .
            ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0' . $cats_sql . $type_sql .
            ' ORDER BY g.sort_order, g.last_update DESC' . $limit_sql;
