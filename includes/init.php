...
{
    /* 首页分类导航 */
    $nav_category = $GLOBALS['db']->getAll("SELECT cat_id, cat_name FROM " .$GLOBALS['ecs']->table("category") . " where `show_in_nav`=1");
    if (!empty($nav_category))
    {
        $nav_cat_info = array();
        foreach ($nav_category as $key => $value)
        {
            $nav_cat_info[$key]['cat_id'] = $value['cat_id'];
            $nav_cat_info[$key]['cat_name'] = $value['cat_name'];
            $nav_cat_info[$key]['cat_url'] = SITE_URL . "category.php?id=" . $value['cat_id'];
        }
        return $nav_cat_info;
    } else
    {
